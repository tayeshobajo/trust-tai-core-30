...
describe("Ops launch", () => {
  it("fails closed when there is no session and never opens a window", async () => {
    const h = harness();
    const result = await launchOps({ accessToken: null, host: h.host });
  ...
  it("opens the exact Ops SSO origin with no token in the URL or window name", async () => {
    const h = harness();
    const launch = launchOps({ accessToken: TOKEN, host: h.host });
  ...
  it("carries the canonical project id when launched from a project", async () => {
    const h = harness();
    const launch = launchOps({ accessToken: TOKEN, canonicalProjectId: "proj-77", host: h.host });
    h.fire({ origin: OPS_ORIGIN, data: { type: OPS_READY_MESSAGE } });
    await launch;
  ...
  it("reports a blocked popup instead of navigating with a token", async () => {
    const h = harness();
    h.host.blocked = true;
    const result = await launchOps({ accessToken: TOKEN, host: h.host });
