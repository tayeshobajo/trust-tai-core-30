...
function belief(partial: Partial<MemoryBelief> & { id: string }): MemoryBelief {
  return {
    ...
    meta: { kind: "responsibility", facet: "other", patternKey: "engine:comms_silence", outcome: "confirmed" },
    ...partial,
  } as MemoryBelief;
}
...
describe("learningTrail", () => {
  ...
  it("ignores beliefs that did not come from an engine decision", () => {
    const trail = learningTrail([
      belief({ id: "x", meta: { kind: "responsibility", facet: "other", patternKey: "steward:role", outcome: "confirmed" } }),
    ]);
    expect(trail.entries).toHaveLength(0);
  });
  ...
  it("suppresses a shape only after the threshold of dismissals", () => {
    const dismissal = (id: string, at: string) =>
      belief({
        id,
        recordedAt: at,
        meta: { kind: "responsibility", facet: "other", patternKey: "engine:comms_silence", outcome: "dismissed_as_context" },
      });
  ...
  it("records an edit as adopted wording", () => {
    const trail = learningTrail([
      belief({
        ...
        meta: { kind: "responsibility", facet: "other", patternKey: "engine:comms_silence", outcome: "edited_then_confirmed" },
      }),
    ]);
