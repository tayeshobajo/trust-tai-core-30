...
t(
  "Claude URL stays import_needs_upload",
  syncStateFor("claude") === "import_needs_upload",
  "syncStateFor('claude') refuses to pretend readability",
);
t(
  "ChatGPT URL stays import_needs_upload",
  syncStateFor("chatgpt") === "import_needs_upload",
  "syncStateFor('chatgpt') refuses to pretend readability",
);
...
// 2. Invalid ChatGPT link → no fake import (ingest requires actual content)
{
  const { data: bad } = await sb
    ...
    .insert({
      ...
      url: "https://chatgpt.com/c/nonexistent-xyz",
      is_primary: false,
      sync_state: "import_needs_upload",
    })
    .select("id")
    .single();
  const stillNeedsUpload =
    bad &&
    (await sb.from("project_thinking_sources").select("sync_state").eq("id", bad.id).single()).data
      ?.sync_state === "import_needs_upload";
  t(
    "Invalid ChatGPT link → no fake import",
    Boolean(stillNeedsUpload),
    "source remains import_needs_upload without content",
  );
...
// 3. Cross-org source → inaccessible (packet loader scopes by org)
{
  ...
  const { data: foreign } = await sb
    ...
    .maybeSingle();
  t(
    "Cross-org source → inaccessible",
    foreign === null,
    "org-scoped read of fixture project under foreign org returns nothing",
  );
}
...
// 4. Superseded mockup excluded from canonical context
{
  const { data: file } = await sb
    ...
    .insert({
      organization_id: ORG,
      project_id: P,
      name: "intake-mockup-v0.png",
      kind: "reference",
      storage_path: "projects/97184a93/intake-mockup-v0.png",
  ...
  t(
    "Superseded mockup excluded",
    packet.approvedAssets.length === 0,
    "superseded asset never enters approvedAssets",
  );
...
// 5. Draft mockup not treated as approved
{
  const { data: file } = await sb
    ...
    .insert({
      ...
      name: "draft.png",
      kind: "reference",
      storage_path: "projects/97184a93/draft.png",
      content_type: "image/png",
      size_bytes: 0,
    })
    .select("id")
  ...
  t(
    "Draft mockup not approved",
    packet.approvedAssets.length === 0,
    "draft asset never enters approvedAssets",
  );
  await sb.from("project_assets").delete().eq("id", asset!.id);
  await sb.from("project_files").delete().eq("id", file!.id);
}
...
// 6. No GitHub reader → stays linked (nothing flips status without genuine read)
{
  const { data: conn } = await sb
    ...
    .eq("project_id", P)
    .eq("connection_type", "github")
    .single();
  t(
    "GitHub stays linked without reader",
    conn?.status === "linked" && !conn?.last_synced_at,
...
// 7. Agent lacks capability → assignment refused (agentCanTake semantics)
{
  ...
  t(
    "Capability mismatch refused",
    agentCanTake(fakeAgent, fakeTask) === false,
    "comms agent cannot take db schema task by keyword gate",
  );
}
...
// 8. Thinking import offline → confirmed context remains usable (packet builds from DB alone)
{
  ...
  t(
    "Offline import → context usable",
    packet.requirements.length === 1,
    "confirmed knowledge serves packets without live sources",
  );
}
...
// 9. Normalizer: provider-neutral output + hash stability
{
  ...
  t(
    ...
    doc.content_hash === doc2.content_hash,
    "same content → same fingerprint",
  );
  t(
    "Normalizer splits speakers",
    doc.messages.length === 2 && doc.messages[1].role === "assistant",
    "user/assistant turns parsed",
  );
}
