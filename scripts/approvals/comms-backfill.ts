/**
 * Production Comms intake catch-up. OPERATIONS ONLY.
 *
 * Runs the real `backfillCommsApprovals()` source-adapter path against the
 ...
 *
 * Run: bun scripts/approvals/comms-backfill.ts <organizationId> <userId>
 */
...
if (!organizationId || !userId) {
  console.error("FAIL: usage: bun scripts/approvals/comms-backfill.ts <organizationId> <userId>");
  process.exit(1);
}
...
const { backfillCommsApprovals } = await import("../../src/data/approvals/intake");

const report = await backfillCommsApprovals({ organizationId, userId });
