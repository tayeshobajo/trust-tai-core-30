/**
 * Production Scout intake catch-up. OPERATIONS ONLY.
 *
 * Runs the real `backfillScoutApprovals()` source-adapter path against the
 ...
 *
 * Run: bun scripts/approvals/scout-backfill.ts <organizationId> <userId>
 */
...
if (!organizationId || !userId) {
  console.error("FAIL: usage: bun scripts/approvals/scout-backfill.ts <organizationId> <userId>");
  process.exit(1);
}
...
const { backfillScoutApprovals } = await import("../../src/data/approvals/scout-intake");

const report = await backfillScoutApprovals({ organizationId, userId });
...
for (const row of (data ?? []).slice(0, 3)) {
  ...
  const payload = row.payload as { prospectId?: string } | null;
  console.log(
    `  ${row.source_key} | ${row.status} | rev ${row.revision} | ${entity?.type}:${entity?.id} | prospect ${payload?.prospectId}`,
  );
}
