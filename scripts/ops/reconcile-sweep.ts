...
if (!PAPERCLIP_BOARD_KEY || !SUPABASE_URL || !SERVICE_ROLE_KEY) {
  console.error(
    "reconcile-sweep: missing env (PAPERCLIP_BOARD_KEY / TRUST_TAI_SUPABASE_URL / TRUST_TAI_SUPABASE_SERVICE_KEY)",
  );
  process.exit(1);
}
...
async function main() {
  ...
  for (const row of agentRows) {
    ...
    try {
      ...
      const doneRes = await pcGetSafe<
        Array<{ id: string; status: string; title: string; completedAt?: string | null }>
