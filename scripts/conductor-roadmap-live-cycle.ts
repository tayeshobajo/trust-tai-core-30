...
import {
  readRoadmapCanon,
  describeRoadmapCanon,
} from "../src/data/intelligence/conductor/roadmap-cycle";
...
import { loadLearning, loadObservations } from "../src/data/supabase/conductor-learning-service";
  ...
  data:
    (preferred
      ? memberships.data.find(
          (row) => String((row as Record<string, unknown>)["organization_id"]) === preferred,
        )
...
log(
  "roadmaps",
  roadmaps.map((r) => ({ id: r.id, title: r.title, subject: r.subjectLabel, status: r.status })),
);
const target =
...
log("roadmap", {
  ...
  pointA: detail.roadmap.pointA.map((n) => ({
    label: n.label,
    tier: n.tier,
...
log(
  "evidence",
  answer.evidence.map((e) => e.label),
);
log("roadmapCanon", answer.roadmapCanon ?? null);
log(
  "actionGraph ops",
  (answer.actionGraph?.actions ?? []).map((a) => ({
    app: a.owningApp,
    op: a.operation,
...
log(
  "follow-up proposedActions",
  followUp.proposedActions.map((a) => ({ app: a.owningApp, op: a.operation, title: a.title })),
);
log(
  "follow-up actionGraph ops",
  (followUp.actionGraph?.actions ?? []).map((a) => ({ app: a.owningApp, op: a.operation })),
);
...
const foreign = await supabase
  ...
  .limit(5);
log("foreign roadmaps visible", {
  count: foreign.data?.length ?? 0,
  error: foreign.error?.message ?? null,
});
