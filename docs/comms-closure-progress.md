...
| C09 / C13 / P4.5 (private notes) | Proposal revised, **then applied** | Sandbox, 2026-09-16; applied by Codex 2026-09-15 | `comms_review_runs.opportunities` | **Schema availability closed.** The earlier `NOT NULL DEFAULT '[]'` would have relabelled every historical run as "none raised". The column is nullable with an array-shape constraint: NULL means *not recorded*, `[]` means *the run looked and raised none*, a populated array means notes. Completed runs write the array explicitly. Reading, reload and write-failure behaviour tested in `src/lib/comms-review-private-notes.test.ts`; run immutability and RLS untouched. Tai approved the exact Revision 2 SQL; Codex applied it as `comms_review_opportunities_nullable` and verified the nullable column, the constraint, the untouched historical NULL, the unchanged record digest, and RLS. Archived SQL: `docs/migrations/20260915120000_comms_review_opportunities.sql`. **Still open:** a real completed run persisting and reloading notes in the authenticated workspace | Codex (live persistence/reload check) |
...
Opportunities proposal (`20260915120000_comms_review_opportunities.sql`, Revision 2): **applied 2026-09-15** by Codex under the migration name `comms_review_opportunities_nullable`, after Tai's explicit approval of that exact revision following the automatic review refusal. Post-apply verification: `opportunities` is nullable jsonb with default null and the array-shape constraint present; the one historical run retains NULL; the original-record digest is identical before and after (`0773ed95b95a1259255afbd365cfb1f9`); RLS remains enabled and authenticated UPDATE remains denied. Archived to `docs/migrations/20260915120000_comms_review_opportunities.sql`; **do not reapply**. This closes **schema availability only** — live model persistence and reload verification of private notes remain open. The lessons proposal (`20260916090000_comms_review_lessons.sql`) stays unapplied pending review.

### Lessons hardening, Revision 3 (Codex review at b9f9954) — code and proposal only, nothing applied

Build/environment: this Lovable preview against the external project `okydosoacqdnursmmenf`. No migration applied, no publish, no send, no new live evidence. QA session `9b329dbe-02ca-40d6-9f54-9ec728c64446` untouched.

| Acceptance row | Evidence type | Result | Remaining owner |
| --- | --- | --- | --- |
| L15 — A database without `kept_lessons_snapshot` must not lose voice provenance | Test (`comms-review-lessons.server.test.ts`, "drops only kept_lessons_snapshot, never the voice provenance"): the insert double refuses only that named column, as a real workspace of that shape would; the retry still carries `voice_profile_id`, `voice_version`, `voice_snapshot_checksum`, `style_context_snapshot` and the lessons stamp in `stages`, and the run completes | PASS-CODE | — |
| L16 — The column exists and old runs read as "not recorded" | Proposed SQL: `ADD COLUMN IF NOT EXISTS kept_lessons_snapshot jsonb`, nullable, no default, with a comment saying NULL means not recorded rather than none in force | PASS-CODE (proposal) — **not applied** | Tai approval, then Codex application |
| L17 — No inherited DELETE/TRUNCATE survives on the lessons table | Proposed SQL: `REVOKE ALL` now also from `service_role` before the minimal grants | PASS-CODE (proposal) — **not applied** | Tai approval, then Codex application |
| L18 — Deleting an audit source must not remove history | Proposed SQL: both composite source FKs changed to `ON DELETE RESTRICT`; only `organizations` still cascades, which is deliberate workspace teardown | PASS-CODE (proposal) — **not applied** | Tai approval, then Codex application |
| L19 — The private note and the revocation pair are frozen | Proposed SQL trigger: `private_note` added to the immutable set; once revoked, neither `revoked_at` nor `revoked_by` may change at all | PASS-CODE (proposal) — **not applied** | Tai approval, then Codex application |
| L20 — Stopping a habit is idempotent and distinguishes "no such habit" | Tests: an already-stopped habit returns the original record with no write; an absent one refuses with `not_found`; a live one is stopped by a conditional `revoked_at IS NULL` update and the record is handed back | PASS-CODE | — |
| L21 — The snapshot and hash name the catalogue version, the lesson ids and the exact wording | Code (`LESSON_CATALOGUE_VERSION`, `lessonSetStamp` over version + ids + categories + promotion times + exact guidance) + test asserting the snapshot's `catalogueVersion` and per-lesson `{id, category, guidance, promotedAt}`. Changed wording therefore changes the stamp and invalidates earlier evidence | PASS-CODE | — |
| L22 — The claim-time race, not only the preflight | Code: `requireCurrentContextAfterClaim` wired into both provider paths (`comms-resend-send.server.ts`, `comms-gmail-send.server.ts`) immediately after `claimDelivery` and before any provider request. Test (`comms-send-authority.server.test.ts`): the context moves after the attempt is recorded → refused, provider never called, attempt settled `failed` | PASS-CODE | — |
| L23 — Live verification of L15–L22 | — | BLOCKED (table not applied; this environment cannot sign in) | Tai approval → Codex application → Codex live check |

Verification run for this revision: typecheck clean, 3,008 tests pass across 270 files, build OK. No live run is claimed.
